		...
		if ($('.header .not-auth-box').length > 0) {
			...
			function myTimer() {
				...
				if($('.modal_signup.modal_signup_image').length > 0 && $('html').find('.signup_modal_boxtitle').length < 1) {
					...
					$('.modal_signup').addClass('modal_signup_image');
					$('.modal_signup_image .signup-user_modal').append('<div class="signup_modal_boxtitle"><span class="signup_modal_title">Створіть свій аккаунт для збереження зображення</span></div>');
			...
			function myTimerBtn(element) {
				...
				if($('.modal_signup.modal_signup_image').length > 0 && $('html').find('.signup_modal_boxtitle').length < 1) {
					...
					$('.modal_signup').addClass('modal_signup_image');
					var title = typeof element.title != 'undefined' ? element.title : 'Зареєструйтесь, щоб додати це зображення до обраного';
