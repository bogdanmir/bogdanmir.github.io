try{
    hj('trigger', 'optimize_pdp');
}catch (e) {}
function initjQuery(callback) {
    var script = document.createElement( "script" )
    script.type = "text/javascript";
    if(script.readyState) {
        script.onreadystatechange = function() {
            if ( script.readyState === "loaded" || script.readyState === "complete" ) {
                script.onreadystatechange = null;
                callback();
            }
        };
    } else {
        script.onload = function() {
            callback();
        };
    }
    script.src = '//code.jquery.com/jquery-3.3.1.min.js';
    document.getElementsByTagName( "head" )[0].appendChild( script );
}
if (!window.jQuery) {
    initjQuery(function() {
        readyjQueryinit();
    });
}else{
    readyjQueryinit();
}

function readyjQueryinit() {
	jQuery(function($) {

	window.dataLayer = window.dataLayer || [];
		dataLayer.push({
		'event': 'event-to-ga',
		'eventCategory': 'Exp - Optimize PDP UX',
		'eventAction': 'loaded'
	});
	$(document).on("click", ".product_section .swatch_options .swatch .swatch-element:nth-child(1)", function () {
		console.log('QUANTITY - 1');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'QUANTITY - 1 PACK'
		});
	});
	$(document).on("click", ".product_section .swatch_options .swatch .swatch-element:nth-child(2)", function () {
		console.log('QUANTITY - 2');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'QUANTITY - 3 PACK'
		});
	});
	$(document).on("click", ".product_section .swatch_options .swatch .swatch-element:nth-child(3)", function () {
		console.log('QUANTITY - 3');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'QUANTITY - 3 PACK auto-ship'
		});
	});
	$(document).on("click", ".on-qty-wrapper .on-qty-plus", function () {
		console.log('QUANTITY +1');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'QUANTITY +1'
		});
	});
	$(document).on("click", ".on-qty-wrapper .on-qty-minus", function () {
		console.log('QUANTITY -1');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'QUANTITY -1'
		});
	});

	$(document).on("click", ".on-payment-wrapper .on-item-1", function () {
		console.log('Apple Pay');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'Payment method - https://cdn.shopify.com/s/files/1/2572/8006/files/Group_259.png?v=1576496655'
		});
	});
	$(document).on("click", ".on-payment-wrapper .on-item-3", function () {
		console.log('PayPal');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'Payment method - https://cdn.shopify.com/s/files/1/2572/8006/files/Group_258.png?v=1576496694'
		});
	});
	$(document).on("click", ".on-payment-wrapper .on-item-2", function () {
		console.log('Google Pay');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'Payment method - https://cdn.shopify.com/s/files/1/2572/8006/files/Group_260.png?v=1576496678'
		});
	});
	$(document).on("click", ".on-payment-wrapper .on-item-4", function () {
		console.log('Amazon Pay');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'Payment method - https://cdn.shopify.com/s/files/1/2572/8006/files/Group_257.png?v=1576496708'
		});
	});
	$(document).on("click", ".purchase-details .button--add-to-cart", function () {
		console.log('button-add-to-cart');
		window.dataLayer = window.dataLayer || [];
		dataLayer.push({
			'event': 'event-to-ga',
			'eventCategory': 'PDP',
			'eventAction': 'click',
			'eventLabel': 'Payment method - Pay with card'
		});
	});

	$('.product_section .product__images').append('<span class="made_in">Made in USA</span>');
    $('.product_section .shopify-product-form .button--add-to-cart .text').text('checkout');

	$('.swatch .1-pack-swatch label.on-variant-label .on-title').text('4-week pack');

	$('.swatch .3-pack-swatch label.on-variant-label').prepend('<span class="top-label">TOP-SELLER</span>');
	$('.swatch .3-pack-swatch label.on-variant-label .on-title').text('12-week pack');
	$('.swatch .3-pack-swatch label.on-variant-label').append('<span class="save-label">Save 7%</span>');
	$('.swatch .3-pack-swatch label.on-variant-label').append('<span class="old-price-label">$59.91</span>');
	$('.swatch .3-pack-swatch label.on-variant-label').append('<span class="new-price-label">$55.97</span>');
	$('.swatch .3-pack-swatch label.on-variant-label').append('<span class="weeks-label">per 12 weeks</span>');

	$('.swatch .3-pack-auto-ship-swatch label.on-variant-label').prepend('<span class="best-label">best deal</span>');
	$('.swatch .3-pack-auto-ship-swatch label.on-variant-label .on-title').text('Subscribe');
	$('.swatch .3-pack-auto-ship-swatch label.on-variant-label').append('<span class="save-label">Save 17%</span>');
	$('.swatch .3-pack-auto-ship-swatch label.on-variant-label').append('<span class="old-price-label">$59.91</span>');
	$('.swatch .3-pack-auto-ship-swatch label.on-variant-label').append('<span class="new-price-label">$49.97</span>');
	$('.swatch .3-pack-auto-ship-swatch label.on-variant-label').append('<span class="weeks-label">every 12 week</span>');

	var styles = "<style>";
	styles += `
		.product_section .product__images {
			position: relative;
		}
		.product_section .product__images .made_in{
			position: absolute;
			top: 63px;
			right: 0;
			font-weight: 500;
    		font-size: 12px;
		}
		.product_section .product__images .made_in:before{
			content: '';
			display: inline-block;
			background-image: url("data:image/svg+xml;base64,");
			height: 10px;
			width: 14px;
			margin-right: 7px;
			border-radius: 1px;
		}
		.product_section .shopify-product-form {
			padding: 23px 25px 18px;
		}
		.product_section .shopify-product-form .button--add-to-cart .text {
			font-weight: 900;
			font-size: 15px;
		}
		.product_section .product-form-container.has-padding-top {
			padding-top: 0;
		}
		.product_section .swatch label.on-variant-label .on-save {
			display: none;
		}
		.product_section .swatch label.on-variant-label .on-title {
			color: #1E415F;
			font-weight: 900;
			font-size: 16px;
			line-height: 19px;
			text-transform: none;
			margin: 6px 0 10px;
		}
		.swatch .top-label{
			background: #F2B413;
			border-radius: 4px;
			color: #FFFFFF;
			text-transform: uppercase;
			font-weight: 900;
			font-size: 9px;
			line-height: 15px;
			width: 76px;
			margin: 0 auto;
			display: block;
			position: relative;
			margin: 0 auto 7px;
		}
		.swatch .top-label:before {
			content: '';
			display: inline-block;
			background-image: url("data:image/svg+xml;base64,");
			height: 9px;
			width: 9px;
			margin-right: 7px;
		}
		.swatch .save-label {
			display: block;
			color: #fff;
			background-image: url("data:image/svg+xml;base64,");
			width: 58px;
			height: 15px;
			font-size: 10px;
			line-height: 12px;
			display: flex;
			align-items: center;
			justify-content: center;
			margin: 0 auto 9px;
		}
		.swatch .old-price-label {
			display: block;
			line-height: 1;
			color: #757575;
			font-weight: 500;
			font-size: 10px;
			text-decoration: line-through;
			display: none;
			margin-bottom: 4px;
		}
		.swatch .new-price-label {
			display: block;
			color: #1E4670;
			font-weight: 500;
			font-size: 14px;
			line-height: 16px;
			display: none;
		}
		.swatch .weeks-label {
			display: block;
			color: #1E4670;
			line-height: 1;
			font-size: 12px;
			line-height: 16px;
			text-transform: none;
			display: none;
		}
		.swatch .best-label {
			background: #F27113;
			border-radius: 4px;
			color: #FFFFFF;
			text-transform: uppercase;
			font-weight: 900;
			font-size: 9px;
			line-height: 15px;
			width: 76px;
			margin: 0 auto;
			display: block;
			position: relative;
			margin: 0 auto 7px;
		}
		.swatch .best-label:before {
			content: '';
			display: inline-block;
			background-image: url("data:image/svg+xml;base64,");
			height: 8px;
			width: 6px;
			margin-right: 7px;
		}
		.product_section .swatch_options .swatch {
			align-items: flex-start;
			margin-left: -7.5px;
			margin-right: -7.5px;
		}
		.product_section .swatch .swatch-element {
			background: #FFFFFF;
			border: 1px solid #C4C4C4;
			border-radius: 10px;
			width: calc(33.33% - 15px);
			margin-right: 7.5px;
			margin-left: 7.5px;
		}
		.product_section .swatch label.on-variant-label {
			padding: 9px 9px;
		}
		.product_section .swatch_options input:checked+.swatch-element {
			background: #F1F7FC;
			border: 1px solid #4090D1;
		}
		.product_section .swatch_options input:checked+.swatch-element label.on-variant-label {
			background: #F1F7FC;
		}
		.product_section .swatch_options input:checked+.swatch-element .old-price-label,
		.product_section .swatch_options input:checked+.swatch-element .new-price-label,
		.product_section .swatch_options input:checked+.swatch-element .weeks-label {
			display: block;
		}
		.on-subscribe-wrapper {
			display: none;
		}
		@media only screen and (max-width: 1244px){
			.product_section .swatch label.on-variant-label .on-title {
				font-size: 14px;
			}
			.product_section .swatch .swatch-element {
				width: calc(33.33% - 10px);
				margin-right: 5px;
				margin-left: 5px;
			}
			.product_section .swatch_options .swatch {
				margin-left: -5px;
				margin-right: -5px;
			}
		}
	`;
	styles    += "</style>";
	$('body').append(styles);
	});
}