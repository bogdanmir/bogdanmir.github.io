		...
		if ($('.header .not-auth-box').length > 0) {
			...
			function myTimer() {
				...
				if($('.modal_signup.modal_signup_image').length > 0 && $('html').find('.signup_modal_boxtitle').length < 1) {
					...
					$('.modal_signup').addClass('modal_signup_image');
					$('.modal_signup_image .signup-user_modal').append('<div class="signup_modal_boxtitle"><span class="signup_modal_title">Создайте аккаунт для сохранения картинки</span></div>');
			...
			function myTimerBtn(element) {
				...
				if($('.modal_signup.modal_signup_image').length > 0 && $('html').find('.signup_modal_boxtitle').length < 1) {
					...
					$('.modal_signup').addClass('modal_signup_image');
					var title = typeof element.title != 'undefined' ? element.title : 'Зарегистрируйтесь, чтобы добавить это изображение в избранное';
			...
			$('body').on('mousedown','.modal-container .price-table-classic__download', function(){
				if($('html').find('.view-file-box_video .video-playing').length < 1){
					myStopFunction();
